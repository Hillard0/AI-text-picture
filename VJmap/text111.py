...
def find_outermost_coordinates(filename):
    ...
    for entity in msp:
        ...
        if min_y <  1e-10 :
            min_y = 0
        #print(min_x, min_y, max_x, max_y ,f )
    ...
    try:
        ...
        for entity in msp:
            if entity.dxftype() == 'POINT':
                ...
                if is_inside_bbox(start_coord[0], start_coord[1], bbox) or is_inside_bbox(end_coord[0], end_coord[1],
                                                                                          bbox):
                    line_coords = {
                        "start": start_coord,
                        "end": end_coord,
                ...
                if any(is_inside_bbox(point[0], point[1], bbox) for point in spline_coords):
                    spline_length = sum(calculate_distance(spline_coords[i], spline_coords[i + 1]) for i in
                                        range(len(spline_coords) - 1))
...
def find_linear_dimension_in_moving_bbox(dxf_file_path, initial_bbox, output_json_path, step_size=ymax - ymin):
    ...
    while not linear_dimensions:
        ...
        if linear_dimensions:
            ...
            if linear_dimensions_up:
                coords["linear_dimensions"] = linear_dimensions_up
                save_to_json(coords, output_json_path)
                print(f"Linear dimension found within bbox: {bbox_up}")
                break
            ...
            if linear_dimensions_down:
                coords["linear_dimensions"] = linear_dimensions_down
                save_to_json(coords, output_json_path)
                print(f"Linear dimension found within bbox: {bbox_down}")
                break

            bbox = bbox_down  # 更新bbox为向下移动后的bbox

# Find linear dimension within moving bounding box and save to JSON
find_linear_dimension_in_moving_bbox(dxf_file_path, initial_bbox, output_json_path)
...
print(type(coords))
print(type(coords["linear_dimensions"]))
print(f"底板标注为 ： {coords['linear_dimensions']}")